    ...
    this.dots.forEach((dot, index) => {
      ...
      if(!outx&&!outy){
        while(!this.checkDotLocation(dot) && !dot.x &&!dot.y){
            ...
            let innerLocation = getLocation(centerX, centerY, outerDeg, outerR);
           dot = Object.assign(dot,innerLocation);
        }
        this.dotsLocations[id] = {x:dot.x,y:dot.y};
      }
      const {x:cx,y:cy} = dot;
      dot.relationDots.forEach((relationId)=>{
          let relationDot = {id:relationId}
        while(!this.checkDotLocation(relationDot) && !relationDot.x &&!relationDot.y){
            outerDeg+=15;
            if(outerDeg>=365){
                outerDeg = 0;
                outerR +=r;
            }
            let innerLocation = getLocation(cx, cy, outerDeg, outerR);
            relationDot = Object.assign(relationDot,innerLocation);
        }

        this.dotsLocations[relationId] = {x:relationDot.x,y:relationDot.y};
        //   let endLocation = getLocation(cx, cy, minDeg, r);
          console.log(relationDot)
      })
      debugger;
