class Force{
    ...
    initNodes(){
        ...
        this.dots.forEach(dot=>{
            ...
            console.log(dot.relationDots,centerDot)
            operationDot(dot.relationDots,centerDot,minDeg,dotR)
        })
    }
}
function operationDot(dots,center,deg,r){
    const {cx,cy} = center;
    console.log(cx,cy,deg,r)
    dots.forEach(dot=>{
        const {x,y} = getLocation(cx,cy,deg,r);
        console.log(x,y)
        dot = Object.assign(dot,getLocation(cx,cy,deg,r));
    })
}
