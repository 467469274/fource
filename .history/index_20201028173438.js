...
function dragged(d) {
  const { x, y } = d3.event;
  if (this.inBoundaries(x, y, false)) {
    d.fx = x;
    d.fy = y;
  }
  // d.fx = d3.event.x;
...
function inBoundaries(x, y, bool) {
  const { iconSize: is, width, height } = this.svg;
  const r = (is / this.initialScale) * (1 + 1.2 * Number(bool));
  const { k, x: tx, y: ty } = bool ? { k: 1, x: 0, y: 0 } : this.trans;
  const newX = (x + is) * k + tx;
  const newY = (y + is) * k + ty;

  return (
    newX >= this.distance.tab + r * k &&
    newX <= width - r * k &&
    newY >= r * k &&
    newY <= height - r * k
  );
