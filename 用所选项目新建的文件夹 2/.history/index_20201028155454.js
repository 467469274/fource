...
function getMaxNumber(){
  let [left, right, top, bottom] = [1e9, -1e9, 1e9, -1e9];
  ars.forEach(node => {
    // console.log(node)
    if (minX > initialBBox(node).left) {
      minX = initialBBox(node).left
    }
    if (maxX < initialBBox(node).right) {
      maxX = initialBBox(node).right
    }
    if (minY > initialBBox(node).top) {
      minY = initialBBox(node).top
    }
    if (maxY < initialBBox(node).bottom) {
      maxY = initialBBox(node).bottom
    }
  });
  nodeRangInfo = {left, right, top, bottom};
  console.log(nodeRangInfo)
  // ars
}
// 得到 circle 的四条边
