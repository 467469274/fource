...
function drawThumb() {
  ...
  thumbSvg = d3
    ...
    .attr("viewBox",function(){
      if(!nodeRangInfo){
        ...
        w = ((Math.abs(left)+Math.abs(right))*1.5)/5;
        h =((Math.abs(top)+Math.abs(bottom))*1.5)/5;
        let floatX = left*1.5/5;
        let floatT = top*1.5/5*1.5;
        x = floatX>0?floatX:0;
        y = floatT>0?floatT:0;
      }
      return `${x} ${y} ${w} ${h}`
  ...
  thumbPath.attr('transform',function(d){
    console.log(d);
    return `translate(${d.x *1.5/5},${d.y *1.5/5})`
  });
  thumbPath.attr("d", (d) => {
